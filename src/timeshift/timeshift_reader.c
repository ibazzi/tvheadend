    ...
    streaming_message_t **sm, int *wait )
{
  ...
  if (tsf) {
...
#if ENABLE_ANDROID
    tvhtrace("timeshift", "ts %d seek to %ld (fd %i)", ts->id, (long)tsf->roff, tsf->rfd);
#else
...
#if ENABLE_ANDROID
    tvhtrace("timeshift", "ts %d read msg %p (%ld)", ts->id, *sm, (long)r);  // Android bug, ssize_t is long int
#else
    ...
    if (r == 0) {
      if (tsf->rfd >= 0) {
#if ENABLE_ANDROID
        tvhtrace("timeshift", "ts %d seek to %ld (fd %i) (incomplete)", ts->id, (long)tsf->roff, tsf->rfd);
#else
